...
sc_expert = ax1.scatter(
    merged["Concentration"],
    merged["Time, s_expert"],
    c="red",
    alpha=0.7,
    s=80,
    label="Expert",
...
logs[0] = 10
ax1.plot(x_fit, logs, "r--", label=f"Expert Trend")
...
ax2.text(
    0,
    merged["Time, s_expert"].mean(),
    f'{merged["Time, s_expert"].mean():.0f}s',
    ha="center",
    va="bottom",
    fontsize=10,
)
ax2.text(
    1,
    merged["Time, s_auto"].mean(),
    f'{merged["Time, s_auto"].mean():.1f}s',
    ha="center",
    va="bottom",
    fontsize=10,
)
