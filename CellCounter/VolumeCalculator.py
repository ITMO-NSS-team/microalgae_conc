...
def calc_volume(imgs_path: str, l: float, h: float, P_h: int, P_w: int, plot_stats_path=None):
    """
    Calculates microscope image volume (mL) for automated cell concentration measurement.
    Implements the 3-step calibration procedure from chamber grid to volume conversion.
    ...
    if plot_stats_path is not None:
        ...
        # Main plot
        plt.scatter(np.arange(len(p_list)), p_list, c='r', label='Measured pixel values')
        plt.axhline(p, c='green', linestyle='--', label=f'Mean = {p:.0f} px (s = {s:.1e} mm/px)')
        ...
        for i, p_i in enumerate(p_list):
            ...
            plt.annotate(f'{diff:+.1f}%', xy=(i, mid_y), ha='right', va='center',
                         rotation=90, fontsize=9, c='black')
