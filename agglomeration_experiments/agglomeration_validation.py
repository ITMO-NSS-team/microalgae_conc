...
df['Error (%)'] = (100 * (df['Auto'] - df['Manual']).abs() / df['Manual']).round().astype(int)
...
sc = ax1.scatter(df['Agg. Rate'], df['Error (%)'],
                c=df['Manual']/1e6, cmap='viridis', s=100, alpha=0.8)
...
for i, row in df.iterrows():
    ax1.text(row['Agg. Rate']+2, row['Error (%)'], str(row['Sample_ID']),
...
table_data['Error (%)'] = table_data['Error (%)'].astype(int)  # Already rounded, ensure int
